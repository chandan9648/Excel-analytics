...
const AdminPanel = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await fetch("https://excel-analytics-platform-z594.onrender.com/api/admin/users", {
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      const token = sessionStorage.getItem("token");
      const res = await fetch(`https://excel-analytics-platform-z594.onrender.com/api/admin/users/${id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
